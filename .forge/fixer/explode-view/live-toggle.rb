...
pos = lambda { kids.map { |k| o = k.transformation.origin; [o.x.to_f, o.y.to_f, o.z.to_f] } }
dmax = lambda { |a, b| a.each_index.map { |i| (0..2).map { |j| (a[i][j] - b[i][j]).abs }.max }.max }
out = {}
home = pos.call
begin
  m.selection.clear; m.selection.add(booth)
  WR_ExplodeView.ability_on('mode' => 'Axis', 'spread' => '60', 'fan' => '150')
  a = pos.call
  out['moved_at_60'] = a.each_index.count { |i| dmax.call([a[i]], [home[i]]) > 0.001 }
  WR_ExplodeView.ability_on('mode' => 'Axis', 'spread' => '90', 'fan' => '150')
  b = pos.call
  ...
  %w[Radial Vertical].each do |md|
    m.selection.clear; m.selection.add(booth)
    ok = WR_ExplodeView.ability_on('mode' => md, 'spread' => '60', 'fan' => '150')
    out["#{md}_ran"] = ok
    m.selection.clear
    WR_ExplodeView.ability_off({})
  ...
  if err > 1e-6
    ...
    kids.each_with_index { |k, i| d = Geom::Point3d.new(*home[i]) - k.transformation.origin; k.transform!(Geom::Transformation.translation(d)) }
    m.commit_operation
