  ...
  def self.select_scenes(model, spec)
    ...
    s.split(',').map(&:strip).reject(&:empty?).each do |tok|
      hit = if tok =~ /\A(\d+)\s*-\s*(\d+)\z/
              ...
              # batch. Clamp to the real bounds first, and report whatever fell
              # off either end instead of dropping it silently — the user asked
              # for those numbers and is owed a word about them.
              lo = a < 1 ? 1 : a
