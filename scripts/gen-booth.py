...
# ------------------------------------------------- side-wall panel ORDER --
#
# THIS FILE IS THE ONLY OWNER OF WHICH END OF A SIDE WALL EACH PANEL SITS AT.
#
# Benton, 2026-09-02, off the booth builder's own floor / ceiling / wall
# rendering: "the wide floor section, the door frame and the wide side-wall
# panel all sit at the same end - the door end - on every split-run model."
# One rule, both families, both side walls, both shells:
#
#     MDL 7272 / 7296   46 at y 2..48 (door end), 22 at y 50..72
#     MDL 6060 / 6084   40 at y 2..42 (door end), 16 at y 44..60
#
# That is exactly what the S->N walk below produces when slot 0 - the wide
# slot on every one of these layouts - is laid down first from the door wall,
# so there is NO swap step. There used to be one: 1.7.10 (2026-08-28, commit
# a886105) reversed the 40/16 pairing on the 6060 / 6084 off an earlier
# built-booth ruling, which put slot id E0 at the FAR end on the 60-series and
# the DOOR end on the 72-series. An id-keyed ASSIGN in build-booth-components.rb
# then swapped names as well, and the two composed into a mirrored 7272 on the
# standalone path and a mirrored 6060 on the share-link path (audit 2026-09-01
# finding 1). The 2 Sep ruling supersedes the 28 Aug one; the swap is gone and
# slot ids ascend from the door end on every model.
#
# ASSIGN in build-booth-components.rb may say WHICH part fills a slot. It may
# not move one: scripts/rbtest-side-wall-order.py fails if any E/W name it
# assigns is not the width of its slot, and if the two build paths ever put a
# different width at the same y again.
...
def inner_parts(L, W, H, inner, assign=None):
    ...
    for side in ('N', 'S', 'E', 'W'):
        ...
        if not wall:
            continue
        slots = wall['slots']
        # Slot order is the layout's, on both shells, always - the inner shell
        # walks the same direction as the outer one so the two can never
        # disagree on which end of a wall a slot id owns.
