...
module WR_Deck
  ...
  def self.build(model, parent, spec, dir, kind, wall_h = WALL_H)
    ...
    tiles.each do |t|
      ...
      # ====================================================================
      # THE MIRROR IS AN AUTHORING FACT, AND IT SPLITS ON THE HAND.
      # ====================================================================
      #
      # Benton, 2026-08-28, off freshly built booths:
      #
      #   "the standard ceilings had the hinges in the center, not the outside.
      #    So those need to be rotated 180 degrees"  - MDL 7296 E, 7296 S,
      #    7272 S, 7272 E
      #   "only the 6018 CL side is flipped the wrong way"  - MDL 6060
      #   "84 series, 96 series, 102 series are all fine"
      #
      # The mirror below (1.0 - e) generalised ONE observed part - the 84126's
      # STD8442CL SIDE - across every convention-A ceiling. Its own comment
      # named what would falsify it: "a convention-A ceiling that still reads
      # hinges inboard after the change. Say which booth." Benton has now said
      # which booths, and the split between the ones that are right and the ones
      # that are wrong is EXACTLY the HAND letter on the file name.
      #
      # OBSERVED, from the folder listing of the component library (2026-08-28):
      # every handed Standard deck part in it is
      #
      #   STD6018CL/FL SIDE R,  STD6042CL/FL SIDE L and R,
      #   STD7224CL/FL SIDE R,  STD7248CL/FL SIDE L and R
      #
      # - the 60 and 72 series, and nothing else. STD8442CL SIDE,
      # STD10242CL SIDE, STD9648CL SIDE and every CTR panel carry NO hand, so
      # this condition cannot reach the 84, 96 or 102 series by construction.
      # That is the regression boundary Benton drew, enforced structurally
      # rather than by a model list.
      #
      # WHY IT SPLITS THERE - derived, and this is the part to argue with. An
      # UNHANDED SIDE part is one file serving both ends, and it is its floor
      # twin turned over about the part's LONG axis, which mirrors the short
      # (tiling) axis - so the twin's fraction has to be mirrored to be read on
      # the ceiling. A HANDED family ships the two ends as two files and the
      # pair is generated by mirroring across the TILING axis; turning one of
      # those over about its SHORT axis leaves the short-axis bracket fraction
      # where it was, so mirroring it a second time here points it inboard.
      # The five tiles Benton reports wrong are exactly the five this moves:
      # STD7248CL SIDE L (7272, 7296), STD7248CL SIDE R (7296),
      # STD7224CL SIDE R (7272) and STD6018CL SIDE R (6060). The MDL 6084's
      # STD6042CL pair is untouched - its floor twins measure 0.4301, which
      # bracket_edge calls symmetric, so it has no cue and keeps the positional
      # rule. Benton reports the 6084 ceiling correct, which is that.
      #
      # WHAT FALSIFIES THIS: a handed ceiling tile that still reads hinges
      # inboard, or an 84/96/102 tile that moves at all. Both are visible in the
      # console line each tile prints below, which ends in its edge fraction.
      own  = bracket_edge(defn)
      twin = fl_twin(cat, t[:part])
      handed = !t[:part][:hand].to_s.empty?
      t[:edge] = if own
                   own
